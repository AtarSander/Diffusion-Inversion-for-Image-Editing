...
from editing.edit_audios_flowedit_zome import prepare_dataset
from editing.AudioEditingCode.code.env import PATH_MUSICCAPS
from torchaudio.transforms import Resample
import fire


def prepare_data(task_id: str, path_audio: str):
    if isinstance(task_id, int):
        task_id = str(task_id)
    df_musiccaps = prepare_dataset(task_id, PATH_MUSICCAPS)
    target_prompts = []
    source_audios = []
    edits = []
    srs = []
    for idx, row in df_musiccaps.iterrows():
        ...
        source_audios.append(torchaudio.load(row['path_yt'])[0])
        audio, sr = torchaudio.load(path_audio+f"/a{idx}.wav")
        edits.append(audio)
        srs.append(sr)
    return target_prompts, source_audios, edits, srs
...
def get_lpaps(source_audios, edits, srs,device):
    # load lpaps
    lpaps_model = LPAPS(
        net="clap",
        device=device,
    ...
    lpaps_source_target = {}
    with torch.no_grad():
        for audio_idx in tqdm(range(len(source_audios)), desc="Calculating LPAPS", disable=True):
...
def get_clap(target_prompts, edits, srs, device):
    clap_model = CLAPTextConsistencyMetric(
        model_path=os.path.join("res/clap/pretrained", "music_audioset_epoch_15_esc_90.14.pt"),
        model_arch="HTSAT-base" if "fusion" not in "music_audioset_epoch_15_esc_90.14.pt" else "HTSAT-tiny",
        enable_fusion="fusion" in "music_audioset_epoch_15_esc_90.14.pt",
    ...
    clap_target_targetp = {}
    with torch.no_grad():
        for audio_idx in tqdm(range(len(edits)), desc="Calculating CLAP", disable=True):
...
def get_mulan(target_prompts, edits, srs, device):
    # init mulan
    ...
    with torch.no_grad():
        all_similarities = []
        for audio_idx in tqdm(range(len(edits)), desc="Calculating MUQT", disable=True):
            all_texts = [target_prompts[audio_idx]]
            text_embeds = mulan(texts=all_texts)
            batch_audio = edits[audio_idx]
            batch_audio = Resample(srs[audio_idx], 24000)(batch_audio)
...
def main(path_audio: str, task_id: str):
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    target_prompts, source_audios, edits, srs = prepare_data(task_id, path_audio)
    lpaps_source_target_df = get_lpaps(source_audios, edits, srs, device)
    lpaps_source_target_df.to_csv(path_audio + "/lpaps_to_source.csv")
    clap_target_targetp_df = get_clap(target_prompts, edits, srs, device)
    clap_target_targetp_df.to_csv(path_audio + "/clap_to_target_prompt.csv")
    mulan_target_targetp_df = get_mulan(target_prompts, edits, srs, device)
    mulan_target_targetp_df.to_csv(path_audio + "/mulan_to_target_prompt.csv")
    ...
    print(final_results)
    with open(path_audio + "/final_results.json", "w") as f:
        json.dump(final_results, f)

if __name__ == "__main__":
    # TASK_ID = '0'
    # PATH_AUDIO = "editing/outputs/zome_0/ours_flowedit_20250805110758_nmin0.5_nmax1.0_navg3_gs10.0_steps50"
    fire.Fire(main)
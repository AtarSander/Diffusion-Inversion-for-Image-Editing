...
import torch
import torchaudio
from acestep.apg_guidance import MomentumBuffer, apg_forward, cfg_double_condition_forward, cfg_forward, cfg_zero_star
...
class ACEStepPipeline:
    ...
    def get_text_embeddings(self, texts, text_max_length=256):
        inputs = self.text_tokenizer(
            ...
            truncation=True,
            max_length=self.pad_to_max_len if self.pad_to_max_len is not None else text_max_length,
        )
    ...
    def calc_v(
        ...
        if will_hook:
            ...
            def create_cross_attn_hook(
                target_encoder_hidden_states, target_encoder_attention_mask, target_rotary_freqs_cis
            ):
            ...
            layers_path_parts = [
                layer_to_hook.replace(".ace_step_transformer.", "").split(".") for layer_to_hook in layers_to_hook
            ]
            hook_handles = []
            for layer_path_parts in layers_path_parts:
                ...
                for part in layer_path_parts:
                    if part.isdigit():
                        ...
                        target_transformer_block = getattr(target_transformer_block, part)
    ...
    def ode_inversion_sampling(
        ...
        if do_classifier_free_guidance:
            ...
            if will_hook:
                source_encoder_text_hidden_states = torch.cat(
                    [source_encoder_text_hidden_states, torch.zeros_like(source_encoder_text_hidden_states)], 0
                )
                source_text_attention_mask = torch.cat(
                    [source_text_attention_mask, torch.zeros_like(source_text_attention_mask)], 0
                )
                source_speaker_embds = torch.cat([source_speaker_embds, torch.zeros_like(source_speaker_embds)], 0)
                source_lyric_token_ids = torch.cat(
                    [source_lyric_token_ids, torch.zeros_like(source_lyric_token_ids)], 0
                )
                source_lyric_mask = torch.cat([source_lyric_mask, torch.zeros_like(source_lyric_mask)], 0)
            ...
            target_speaker_embds = torch.cat([target_speaker_embds, torch.zeros_like(target_speaker_embds)], 0)
        ...
        if will_hook and len(layers_to_hook) > 0:
            hook_target_encoder_hidden_states, hook_target_encoder_hidden_mask = self.ace_step_transformer.encode(
                encoder_text_hidden_states=target_encoder_text_hidden_states,
                text_attention_mask=target_text_attention_mask,
            ...
            hook_target_rotary_freqs_cis = self.ace_step_transformer.rotary_emb(
                hook_target_encoder_hidden_states, seq_len=hook_target_encoder_hidden_mask.shape[1]
            )
        ...
        T_steps = infer_steps
        timesteps, T_steps = retrieve_timesteps(scheduler, T_steps, self.device, timesteps=None)
        timesteps = torch.cat((timesteps, torch.tensor([0.0], dtype=self.dtype, device=self.device)))
    ...
    def flowedit_diffusion_process(
        ...
        hook_target_encoder_hidden_states, hook_target_encoder_hidden_mask, hook_target_rotary_freqs_cis = (
            None,
            None,
            None,
        )
    ...
    def text2music_diffusion_process(
        ...
        if add_retake_noise:
            ...
            if not is_repaint:
                target_latents = (
                    torch.cos(retake_variance) * target_latents + torch.sin(retake_variance) * retake_latents
                )
            elif not is_extend:
    ...
    def infer_latents(self, input_audio_path):
        ...
        if actual_frame_length != expected_frame_length:
            ...
            if actual_frame_length < expected_frame_length:
                # Pad
                latents = torch.nn.functional.pad(latents, (0, expected_frame_length - actual_frame_length))
            else:
                # Truncate

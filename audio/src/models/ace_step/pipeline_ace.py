...
class SimpleACEStepPipeline(ACEStepPipeline):
    ...
    def task_edit_ode_inversion(
        ...
        target_lyric_mask = torch.tensor([0]).repeat(batch_size, 1).to(self.device).long()
        if len(edit_target_lyrics) > 0:
            target_lyric_token_idx = self.tokenize_lyrics(edit_target_lyrics, debug=False)
            target_lyric_mask = [1] * len(target_lyric_token_idx)
            target_lyric_token_idx = (
                torch.tensor(target_lyric_token_idx).unsqueeze(0).to(self.device).repeat(batch_size, 1)
            )
    ...
    def task_repaint(
        ...
        assert (
            ref_audio_input is None and audio2audio_enable is False
    ...
    def task_retake(
        ...
        assert src_audio_path is not None, "src_audio_path is not required for retake task"
        src_latents = None
    ...
    def task_audio2audio(
        ...
        assert (
            audio2audio_enable and ref_audio_input is not None
        ), "audio2audio_enable and ref_audio_input are required for audio2audio task"

        add_retake_noise = False

        assert src_audio_path is None, "src_audio_path is not supported for audio2audio task"
        src_latents = None
    ...
    def task_text2music(
        ...
        add_retake_noise = False
        src_latents = None
        assert src_audio_path is None, "src_audio_path is not supported for text2music task"
    ...
    def generate(
        ...
        assert 0 < audio_duration <= 240.0, "audio_duration must be between 0 and 240 seconds"
        assert return_type in {"latent", "audio"}, "Invalid return_type, must be one of: latent, audio, path"
        ...
        encoder_text_hidden_states, text_attention_mask = self.get_text_embeddings(prompt)
        ...
        for lyric in lyrics:
            if len(lyric) > 0:
                ...
                lyric_token_idx = torch.tensor(lyric_token_idx).unsqueeze(0).to(self.device)
                lyric_mask = torch.tensor(lyric_mask).unsqueeze(0).to(self.device)
            else:
                lyric_token_idx = torch.tensor([0]).unsqueeze(0).to(self.device).long()
                lyric_mask = torch.tensor([0]).unsqueeze(0).to(self.device).long()
            lyric_token_ids.append(lyric_token_idx)
            lyric_masks.append(lyric_mask)
        lyric_token_ids = torch.cat(lyric_token_ids, dim=0)
        lyric_masks = torch.cat(lyric_masks, dim=0)
    ...
    def edit_audio_flowedit(
        ...
        assert return_type in {"latent", "audio", "path"}, "Invalid return_type, must be one of: latent, audio, path"
    ...
    def edit_audio_ode_inversion(
        ...
        if return_type == "all_latents":
            output = {
                ...
                "all_inverted_latents": [latent.cpu() for latent in edit_results["all_inverted_latents"]],
                "all_sampled_latents": [latent.cpu() for latent in edit_results["all_sampled_latents"]],
            }

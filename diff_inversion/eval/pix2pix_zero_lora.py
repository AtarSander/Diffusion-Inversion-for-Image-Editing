"""Run Pix2Pix-Zero editing evaluation with LoRA-assisted SD1.5 inversion."""
...
@hydra.main(config_path="../../config", config_name="eval/pix2pix_zero_lora", version_base=None)
def main(cfg: DictConfig) -> None:
    logger.info("Pix2Pix-Zero LoRA eval config:\n{}", OmegaConf.to_yaml(cfg))
    ...
    if not pnp_dir.exists():
        raise FileNotFoundError(f"Pix2Pix-Zero directory does not exist: {pnp_dir}")
    if not (data_path / "mapping_file.json").exists():
        raise FileNotFoundError(f"Pix2Pix-Zero mapping file does not exist: {data_path / 'mapping_file.json'}")
    ...
    logger.info("Running Pix2Pix-Zero command from {}: {}", pnp_dir, " ".join(cmd))

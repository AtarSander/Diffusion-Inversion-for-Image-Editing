"""Run MasaCtrl editing evaluation with LoRA-assisted SD1.5 inversion."""
...
@hydra.main(config_path="../../config", config_name="eval/masactrl_lora", version_base=None)
def main(cfg: DictConfig) -> None:
    logger.info("MasaCtrl LoRA eval config:\n{}", OmegaConf.to_yaml(cfg))
    ...
    if not pnp_dir.exists():
        raise FileNotFoundError(f"MasaCtrl directory does not exist: {pnp_dir}")
    if not (data_path / "mapping_file.json").exists():
        raise FileNotFoundError(f"MasaCtrl mapping file does not exist: {data_path / 'mapping_file.json'}")
    ...
    logger.info("Running MasaCtrl command from {}: {}", pnp_dir, " ".join(cmd))

...
if __name__ == "__main__":
    for idx, model in enumerate(
        ...
        noisess = torch.load(
            f"experiments/outputs_correlation/{model}/T_100/noise.pt", weights_only=False
        ).to(torch.float16)
        latents = torch.load(
            f"experiments/outputs_correlation/{model}/T_100/latents.pt", weights_only=False
        ).to(torch.float16)
        samples = torch.load(
            f"experiments/outputs_correlation/{model}/T_100/samples.pt", weights_only=False
        ).to(torch.float16)

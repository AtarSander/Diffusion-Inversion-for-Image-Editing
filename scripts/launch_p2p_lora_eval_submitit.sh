...
if [[ -z "${HYDRA_SWEEP_DIR:-}" ]]; then
  HYDRA_SWEEP_DIR='/net/pr2/projects/plgrid/plggdiffusion/plgatarsander/logs/p2p_lora_eval/${now:%Y-%m-%d}/${now:%H-%M-%S}'
fi
...
if [[ -x "$PYTHON" ]]; then
  exec "$PYTHON" -m diff_inversion.eval.p2p_lora \
    --config-name eval_p2p_lora_submitit \
    --multirun \
    "hydra.sweep.dir=$HYDRA_SWEEP_DIR" \
    hydra.job.name=p2p_lora_eval \
    "$@"
fi

exec "$UV" run --frozen python -m diff_inversion.eval.p2p_lora \
  --config-name eval_p2p_lora_submitit \
  --multirun \
  "hydra.sweep.dir=$HYDRA_SWEEP_DIR" \
  hydra.job.name=p2p_lora_eval \

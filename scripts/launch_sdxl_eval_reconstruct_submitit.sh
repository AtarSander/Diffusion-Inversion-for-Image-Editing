...
if [[ "${REQUIRE_EVAL_ARTIFACT_DIR:-true}" == "true" ]]; then
  ...
  if [[ "$has_artifact_dir" != "true" ]]; then
    ...
    echo "Set REQUIRE_EVAL_ARTIFACT_DIR=false only if you intentionally want to write into input_dir/sample_*." >&2
    exit 2
  fi
fi
...
exec "${EVAL_PYTHON_CMD[@]}" diff_inversion/eval/reconstruct_sdxl.py \
  --config-name eval_reconstruct_sdxl_submitit \
  --multirun \
  'hydra.sweep.dir=slurm_runs/${now:%Y-%m-%d}/${now:%H-%M-%S}' \
  hydra.job.name=sdxl_eval_reconstruct \
  "$@"

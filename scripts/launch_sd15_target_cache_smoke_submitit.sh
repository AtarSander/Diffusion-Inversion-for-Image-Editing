...
if [[ -x "$PYTHON" ]]; then
  exec "$PYTHON" -m diff_inversion.eval.sd15_target_cache_smoke \
    --config-name eval_sd15_target_cache_smoke_submitit \
    --multirun \
    "hydra.sweep.dir=$HYDRA_SWEEP_DIR" \
    hydra.job.name=sd15_target_cache_smoke \
    "$@"
fi

exec "$UV" run --frozen python -m diff_inversion.eval.sd15_target_cache_smoke \
  --config-name eval_sd15_target_cache_smoke_submitit \
  --multirun \
  "hydra.sweep.dir=$HYDRA_SWEEP_DIR" \
  hydra.job.name=sd15_target_cache_smoke \
